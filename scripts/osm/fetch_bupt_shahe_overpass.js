/*
Fetch OSM data for BUPT Shahe Campus area using Overpass API.

Run:
  node scripts/osm/fetch_bupt_shahe_overpass.js
*/
...
// Broad bbox around BUPT Shahe campus and nearby internal roads/services.
...
async function fetchOverpass() {
  ...
  const osmPath = path.join(outDir, 'bupt_shahe.osm.json')
  const geoPath = path.join(outDir, 'bupt_shahe.geojson')
  ...
  for (const feature of geojson.features) {
    ...
    if (props.amenity !== undefined || props.shop !== undefined || props.tourism !== undefined || props.leisure !== undefined) stats.facilities += 1
    if (props.highway !== undefined) stats.roads += 1
    if (props.indoor !== undefined || props.level !== undefined || props.entrance !== undefined || props.door !== undefined) stats.indoor += 1
  }

  const statsPath = path.join(outDir, 'bupt_shahe_stats.json')
